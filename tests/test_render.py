...
#: pytest's hooks are used as the subject because they are a real hookspec
#: namespace with real names; nothing here depends on pytest being installed.
BASE = DocLinks(
    base_url="https://docs.pytest.org/en/stable/reference/reference.html",
    anchor_prefix="pytest.hookspec",
    namespaces=frozenset({"_pytest.hookspec"}),
)
