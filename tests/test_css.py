...
invisible. Anyone changing the palette should be told by a failing test rather
...
#: with five phases gets the fifth colour, and it has to be as readable as the
#: first - checking only the ones pytest happens to use would leave the rest
#: unverified until somebody declared enough phases to reach them.
PHASE_HUES = hues_for([f"phase{index}" for index in range(len(PALETTE))])
...
def test_fills_run_from_background_toward_the_hue():
    fills = css._fills(PHASE_HUES["phase1"], css.LIGHT_BACKGROUND, css.MAX_TINT_LIGHT)
...
def test_stylesheet_covers_every_phase_and_step_in_both_themes():
    sheet = css.stylesheet(list(PHASE_HUES))
...
def _rendered_svg():
    ...
    links = DocLinks(base_url="https://example/ref.html", anchor_prefix="app.hookspec")
    return dot.render_inline_svg(nodes, {}, links, "phase0", {})
