...
Nothing else uses colour. Changes between versions deliberately do not:
...
from .dot import SHADE_STEPS, hues_for
...
def _phase_rules(hues: dict[str, str], background: Rgb, ceiling: float, prefix: str = "") -> str:
    lines = []
    for phase, hue in hues.items():
...
BASE = """/* Diagrams are inlined SVG so their links stay clickable and CSS can theme
   them. Generated by hook_atlas.render.css - do not edit by hand. */
.ha-diagram {
  overflow-x: auto;
  margin: 1.2rem 0;
}
...
/* The "hide the application's own plugins" toggle. Added by assets/filter.js
   only when a table has something outside the application to show. */
.ha-filter {
  display: inline-flex;
  align-items: center;
  gap: 0.4rem;
...
/* Hook tables. Hook names are long and full of underscores, which browsers
   happily break a hook name mid-word, into "pytest_cmdline" / "_main". */
...
/* Version picker. Readers think in released versions; a site is organised by
...
def stylesheet(phase_keys: Sequence[str] | None = None) -> str:
    """The full diagram stylesheet, light and dark.

    Rules are generated per phase key, so an application declaring its own
    phases gets colours for them. Without keys there is one phase, and it is
    the whole run - which is what an unconfigured application draws.
    """
    hues = hues_for(list(phase_keys) if phase_keys else ["run"])
    return "\n\n".join(
        [
            BASE,
            "/* light theme */",
            f"{MUTED_SELECTOR} {{ fill: #5A5A5A; }}",
            _phase_rules(hues, LIGHT_BACKGROUND, MAX_TINT_LIGHT),
            ...
            _phase_rules(
                hues, DARK_BACKGROUND, MAX_TINT_DARK, prefix='[data-md-color-scheme="slate"] '
            ),
            "",
        ]
    )
