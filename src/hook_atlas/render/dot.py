...
#: Hues assigned to phases in declaration order. Chosen for separation under
#: dichromatic vision, not for convention - the conventional red/amber/green
#: palette puts two of its hues at a colour difference of 3.9 under
#: deuteranopia, which is to say identical. Applications get these in the order
#: they declare their phases, so the first four are what pytest has always had.
PALETTE = (
    "#332288",
    "#56B4E9",
    "#DDCC77",
...
def hues_for(keys: Sequence[str]) -> dict[str, str]:
    """Map phase keys to hues by position, wrapping if there are more than the
    palette holds."""
    return {key: PALETTE[index % len(PALETTE)] for index, key in enumerate(keys)}


#: Used when a caller draws a phase without saying what colour it is - an
#: application nobody has configured, or the fallback whole-run phase. Looking
#: the key up in a dict of pytest's four phase names raised KeyError for
#: anything else, which made an unconfigured application crash the renderer.
DEFAULT_HUE = PALETTE[0]
...
def _column_colours(hue: str, theme: str | None) -> dict[str, str]:
    if theme is None:
        return {}
    from . import css

    background, _ = css.theme_settings(theme)
    title = css.readable_on(hue, background)
...
def build(
    ...
    dot = _new_graph()
    builder = _Builder(hookspecs, links, phase, totals, hue=hue)
...
def build_columns(
    ...
    anchors = anchors or {}
    hues = hues_for([key for key, _, _ in phases])
    for key, title, nodes in phases:
        ...
        with dot.subgraph(name=f"cluster_phase_{key}") as column:
            column.attr(
                ...
                **_column_colours(hues[key], theme),
                **{"class": f"ha-column ha-{key}"},
            )
            builder = _Builder(
                hookspecs, links, key, totals, prefix=f"{key}_", theme=theme, hue=hues[key]
            )
...
def render_inline_svg(
    ...
    return to_inline_svg(build(nodes, hookspecs, links, phase, totals, hue))


def render_svg_file(
    nodes: list[FlowNode], hookspecs: dict[str, Any], links: DocLinks, out_path: str | Path
) -> Path:
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(render_inline_svg(nodes, hookspecs, links))
    return out_path
